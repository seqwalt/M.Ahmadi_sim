...
dt_mons = diff(t_mons);
f_V(t)  = vpa(flip(VC.coefs(end,:))*dt_mons);
g_V1(t) = vpa(flip(Vu1.coefs(end,:))*dt_mons);
g_V2(t) = vpa(flip(Vu2.coefs(end,:))*dt_mons);

f_P(t)  = vpa(flip(PC.coefs(end,:))*dt_mons);
g_P1(t) = vpa(flip(Pu1.coefs(end,:))*dt_mons);
g_P2(t) = vpa(flip(Pu2.coefs(end,:))*dt_mons);

f_A(t)  = vpa(flip(AC.coefs(end,:))*dt_mons);
g_A1(t) = vpa(flip(Au1.coefs(end,:))*dt_mons);
g_A2(t) = vpa(flip(Au2.coefs(end,:))*dt_mons);
